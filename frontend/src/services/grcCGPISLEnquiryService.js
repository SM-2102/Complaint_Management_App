 ...
 * @returns {Promise<Array>} List of warranty enquiry records
 */
async function grcCGPISLEnquiry(params = {}, limit = 100, offset = 0) {
  ...
  const query = Object.entries(mergedParams)
    ...
    .join("&");
  const url = query
    ? `${API_ENDPOINTS.GRC_CGPISL_ENQUIRY}?${query}`
    : API_ENDPOINTS.GRC_CGPISL_ENQUIRY;
