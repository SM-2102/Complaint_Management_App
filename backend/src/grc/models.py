...
class GRC(SQLModel, table=True):
    ...
    grc_no: int = Field(sa_column=Column(pg.INTEGER, nullable=False))
    ...
    received_date: date = Field(sa_column=Column(pg.DATE, nullable=True))
    ...
    status: str = Field(sa_column=Column(pg.VARCHAR(10), nullable=False))
