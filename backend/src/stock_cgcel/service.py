...
from stock_cgcel.models import StockCGCEL, StockCGCELMovement, StockCGCELIndent
from sqlalchemy.sql import func
from stock_cgcel.schemas import StockCGCELGenerateIndentResponse, StockCGCELSchema, StockCGCELEnquiry, StockCGCELEnquiryStockList, StockCGCELIndentCreate, StockCGCELUpdate, StockCGCELGenerateIndentRecord, StockCGCELIndentEnquiry
...
class StockCGCELService:
    ...
    async def enquiry_stock_cgcel(
        ...
        if spare_description:
            statement = statement.where(StockCGCEL.spare_description.ilike(f"{spare_description}"))
        ...
        if available:
            if available == "Y":
                statement = statement.where((StockCGCEL.cnf_qty.isnot(None) & (StockCGCEL.cnf_qty > 0)) |
                                            (StockCGCEL.grc_qty.isnot(None) & (StockCGCEL.grc_qty > 0)) |
                                            (StockCGCEL.own_qty.isnot(None) & (StockCGCEL.own_qty > 0)) )
            else:
                statement = statement.where((StockCGCEL.cnf_qty.is_(None) | (StockCGCEL.cnf_qty == 0)) &
    ...
    async def get_stock_cgcel_by_name(self, spare_description: str, session: AsyncSession):
        statement = select(StockCGCEL).where(StockCGCEL.spare_description == spare_description)
        result = await session.execute(statement)
        spare = result.scalars().first()
        if spare:
            return spare
    ...
    async def update_cgcel_stock(
        ...
        existing_record = await self.get_stock_cgcel_by_code(updateData.spare_code, session)
        ...
        if updateData.movement_type == "SPARE IN":
            existing_record.own_qty = (existing_record.own_qty or 0) + updateData.own_qty
    ...
    async def get_cgcel_indent_details_by_division(self, division: str, session: AsyncSession):
        statement = select(StockCGCEL).where(StockCGCEL.division == division, StockCGCEL.indent_qty.isnot(None), StockCGCEL.indent_qty > 0)
        result = await session.execute(statement)
        rows = result.all()
        indent_details = [
    ...
    async def next_cgcel_indent_code(self, session: AsyncSession):
        statement = select(StockCGCELIndent.indent_number).order_by(StockCGCELIndent.indent_number.desc()).limit(1)
        result = await session.execute(statement)
    ...
    async def generate_cgcel_indent(self, indentData: StockCGCELGenerateIndentRecord, session: AsyncSession, token: dict):
        indent_records = []
    ...
    async def enquiry_indent_cgcel(
        ...
        if spare_description:
            statement = statement.where(StockCGCELIndent.spare_description.ilike(f"{spare_description}"))

        if division:
            statement = statement.where(StockCGCELIndent.division == division)

        if spare_code:
            statement = statement.where(StockCGCELIndent.spare_code.ilike(f"{spare_code}"))

        if from_indent_date:
            statement = statement.where(StockCGCELIndent.indent_date >= from_indent_date)

        if to_indent_date:
            statement = statement.where(StockCGCELIndent.indent_date <= to_indent_date)

        if from_indent_number:
            statement = statement.where(StockCGCELIndent.indent_number >= from_indent_number)

        if to_indent_number:
            statement = statement.where(StockCGCELIndent.indent_number <= to_indent_number)
