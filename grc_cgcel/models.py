...
class GRCCGCEL(SQLModel, table=True):
    ...
    spare_code: str = Field(sa_column=Column(pg.VARCHAR(30), nullable=False, primary_key=True))
    spare_description: str = Field(sa_column=Column(pg.VARCHAR(40), nullable=False))
    grc_number: int = Field(sa_column=Column(pg.INTEGER, nullable=False, primary_key=True))
